...
class PortfolioReadService:
    """
    Read boundary for portfolio identity, holdings, and canonical derived metrics.

    Other modules should consume portfolio state through this service instead of
    ...
    def get_active_holdings(self) -> list[HoldingORM]:
        active = self.get_active_portfolio()
        if active is None:
            return []
        return self.get_portfolio_holdings(active.id)

    def get_portfolio_holdings(self, portfolio_id: int) -> list[HoldingORM]:
        return (
    ...
    # ── Canonical portfolio calculations ─────────────────────────────────────
        ...
        for h in holdings:
            ...
            if hasattr(h, "model_dump"):
                ...
                h_dict = {
                    ...
                    "sector": h.sector,
                    "asset_class": getattr(h, "asset_class", None) or "Equity",
                    "currency": getattr(h, "currency", None) or "INR",
    ...
    @staticmethod
    def compute_summary_from_enriched(
        enriched: list[dict],
        data_source: str,
    ) -> PortfolioSummary:
        if not enriched:
        ...
        total_value = sum(float(h.get("market_value") or 0) for h in enriched)
        total_cost = sum(
        ...
        for h in enriched:
            ...
            sector_values[sector] = sector_values.get(sector, 0.0) + float(h.get("market_value") or 0)
    ...
    @staticmethod
    def compute_sector_allocation_from_enriched(enriched: list[dict]) -> list[SectorAllocation]:
        ...
        for h in enriched:
            ...
            sector_data[sector]["value"] += float(h.get("market_value") or 0)
...
class PortfolioService:
    ...
    async def get_summary(self) -> PortfolioSummary:
        ...
        holdings = await self.provider.get_holdings()
        enriched = self.reader.compute_holding_metrics(holdings)
        return self.reader.compute_summary_from_enriched(enriched, self.provider.mode_name)

    async def get_sector_allocation(self) -> list[SectorAllocation]:
        """Compute sector-level breakdown for pie/donut charts."""
        holdings = await self.provider.get_holdings()
        enriched = self.reader.compute_holding_metrics(holdings)
        return self.reader.compute_sector_allocation_from_enriched(enriched)
    ...
    def _resolve_active_portfolio(self) -> tuple[Optional[int], Optional[str]]:
        ...
        """
        try:
            return self.reader.get_active_portfolio_identity()
        except Exception:
            return None, None
    ...
    async def get_full(self) -> dict:
        ...
        enriched = self.reader.compute_holding_metrics(holdings)
        summary = self.reader.compute_summary_from_enriched(enriched, self.provider.mode_name)
        sectors = self.reader.compute_sector_allocation_from_enriched(enriched)

        # ── Risk snapshot ──────────────────────────────────────────────────────
        risk_snapshot = self.reader.compute_risk_snapshot(enriched, sectors)
